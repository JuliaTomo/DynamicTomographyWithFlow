...
"""
function recon2d_slices_tvrdart!(u::Array{T, 3}, A, b, niter::Int, w_tv::T, nmaterial, epsilon=1e-4) where {T <: AbstractFloat}
    ...
    K = 4*ones(nmaterial-1)
    K = K*ones(nmaterial-1)
    ...
    grays = range(0, stop=1, length=nmaterial)[2:end-1]
